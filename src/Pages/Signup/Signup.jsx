...
function Signup() {
  ...
  const handleSubmitForm = (event) => {
    ...
    const errors = handleValidateFields(inputs); 
    setFormErrors(errors);
    setToSubmit(true);
  
    if (Object.keys(errors).length === 0) {
      axios.post('https://backendflavorfeed.onrender.com/user/registeruser', inputs)
        .then((res) => {
          toast.success(res.data.message);
              ...
              <div className="signup-container  max-w-md  flex flex-col   p-4  text-black bg-white">
                ...
                <form className="flex flex-col gap-3">
                  ...
                  <div className="block ">
                    ...
                    <input type="password" id="password" name="password" placeholder="Password" onChange={inputChange} onClick={() => { setFormErrors({ ...formErrors, password: '' }) }} className="form-input rounded border border-gray-200 text-sm w-full font-normal leading-[18px] text-black tracking-[0px] appearance-none block h-11 m-0 p-[11px] focus:ring-2 ring-offset-2 ring-gray-900 outline-0" />
